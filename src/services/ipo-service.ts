...
export class IPOService {
    ...
    static async createIPO(data: {
        stockId: number;
        totalShares: number; // Lots
        offeringPrice: number;
        listingSessionId?: number;
        startOfferingSessionId?: number;
        endOfferingSessionId?: number;
    }) {
    ...
    static async subscribeIPO(userId: string, ipoId: string, quantity: number) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            if (ipoRes.rows.length === 0) throw new Error('IPO not found');
            ...
            if (sessionRes.rows.length > 0) {
                ...
                if (lastSession.rows.length > 0) currentSessionId = lastSession.rows[0].id;
            }
    ...
    static async finalizeIPO(ipoId: string) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
    ...
    static async getIPOs(status?: string) {
        let query = `
