...
const iep_engine_1 = require("../core/iep-engine");
const market_1 = require("../config/market");
const ipo_service_1 = require("../services/ipo-service");
const dividend_service_1 = require("../services/dividend-service");
const router = (0, express_1.Router)();
// POST /api/admin/init-session - Hitung ARA/ARB berdasarkan harga prev close (ADMIN ONLY)
router.post('/init-session', auth_1.adminAuth, async (req, res) => {
    const { symbol, prevClose } = req.body;
    try {
        const { araLimit, arbLimit } = (0, market_logic_1.calculateLimits)(prevClose);
        res.json({
            symbol,
            prevClose,
            araLimit,
            arbLimit,
            tickSize: (0, market_logic_1.getTickSize)(prevClose)
        });
    }
    catch (err) {
...
router.post('/session/open', auth_1.adminAuth, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
        // 1. Cek apakah sudah ada sesi yang OPEN atau PRE_OPEN
        const existingSession = await client.query(`
            /* dialect: postgres */
            SELECT id FROM trading_sessions WHERE status IN ('OPEN', 'PRE_OPEN')
        `);
        if ((existingSession.rowCount ?? 0) > 0) {
            throw new Error('Sudah ada sesi trading yang sedang berjalan');
        }
        // 2. Buat sesi baru dengan session_number otomatis dan status PRE_OPEN
        ...
        if ((prevSessionRes.rowCount ?? 0) > 0) {
            ...
            if ((offlineOrders.rowCount ?? 0) > 0) {
                ...
                for (const order of offlineOrders.rows) {
                    ...
                    await redis_1.default.zadd(redisKey, order.price, redisPayload);
                }
                ...
                affectedSymbols.forEach((sym) => {
                    matching_engine_1.MatchingEngine.match(sym);
                });
            }
        }
        await client.query('COMMIT');
        // IEP PROCESS (Async Background)
        // Wait for configured duration then Calculate IEP and Open Market
        setTimeout(async () => {
            console.log(`⏱️ PRE_OPEN finished. Calculating IEP...`);
            try {
                // 1. Calculate and Execute IEP for all active stocks
                const activeStocks = await database_1.default.query('SELECT symbol FROM stocks WHERE is_active = true');
                for (const stock of activeStocks.rows) {
                    const iep = await iep_engine_1.IEPEngine.calculateIEP(stock.symbol);
                    if (iep) {
                        await iep_engine_1.IEPEngine.executeIEPTrades(stock.symbol, iep);
                    }
                }
                // 2. Set Session to OPEN
                await database_1.default.query("UPDATE trading_sessions SET status = 'OPEN' WHERE id = $1", [session.id]);
                console.log(`🔔 Market is now OPEN (Session ${session.session_number})`);
                // 3. Trigger Continuous Matching
                for (const stock of activeStocks.rows) {
                    matching_engine_1.MatchingEngine.match(stock.symbol);
                }
                // TODO: Handle IPO Listings here if needed (e.g. check if any IPO listing_session_id matches current session)
            }
            catch (e) {
                console.error("❌ Error during IEP phase:", e);
            }
        }, market_1.MARKET_CONFIG.IEP_DURATION_MS);
        res.json({
            message: `Sesi trading dibuka (PRE_OPEN phase for ${market_1.MARKET_CONFIG.IEP_DURATION_MS / 1000}s)`,
            session
        });
    }
    catch (err) {
...
router.post('/session/close', auth_1.adminAuth, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
        ...
        for (const order of pendingOrders.rows) {
            ...
            if (order.type === 'BUY') {
                ...
                await client.query('UPDATE users SET balance_rdn = balance_rdn + $1 WHERE id = $2', [refund, order.user_id]);
            }
            ...
            // Update status order
            await client.query("UPDATE orders SET status = 'CANCELED' WHERE id = $1", [order.id]);
            // Hapus dari Redis
            const redisKey = `orderbook:${order.symbol}:${order.type.toLowerCase()}`;
        ...
        for (const stock of symbols.rows) {
            ...
            await redis_1.default.del(buyKey);
            await redis_1.default.del(sellKey);
...
router.get('/orderbook/:symbol', auth_1.adminAuth, async (req, res) => {
    try {
        const symbol = req.params.symbol;
        // Ambil dari Redis
        const buyOrders = await redis_1.default.zrevrange(`orderbook:${symbol}:buy`, 0, 19, 'WITHSCORES');
        const sellOrders = await redis_1.default.zrange(`orderbook:${symbol}:sell`, 0, 19, 'WITHSCORES');
        // Parse hasil Redis
...
router.get('/stocks', auth_1.adminAuth, async (req, res) => {
    try {
...
router.put('/users/:userId/balance', auth_1.adminAuth, async (req, res) => {
    try {
        ...
        const user = await auth_service_1.AuthService.adjustUserBalance(userId, amount);
        if (!user) {
...
router.put('/users/:userId/portfolio/:stockId', auth_1.adminAuth, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
...
router.post('/stocks', auth_1.adminAuth, async (req, res) => {
    try {
...
router.put('/stocks/:id', auth_1.adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const circulatingRes = await database_1.default.query('SELECT SUM(quantity_owned) as total FROM portfolios WHERE stock_id = $1', [id]);
        const circulating = parseInt(circulatingRes.rows[0].total || '0');
        res.json({
...
router.post('/stocks/:id/issue', auth_1.adminAuth, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
...
router.get('/users', auth_1.adminAuth, async (req, res) => {
    try {
        const result = await database_1.default.query(`
...
router.get('/orders', auth_1.adminAuth, async (req, res) => {
    try {
...
router.get('/trades', auth_1.adminAuth, async (req, res) => {
    try {
...
router.post('/bot/populate', auth_1.adminAuth, async (req, res) => {
    try {
        const { symbol, minLot, maxLot, spreadPercent, priceLevels } = req.body;
        if (!symbol) {
            return res.status(400).json({ error: 'Symbol diperlukan' });
        }
        const result = await bot_service_1.BotService.populateOrderbook(symbol, {
            minLot,
            maxLot,
            spreadPercent,
            priceLevels
...
router.post('/bot/populate-all', auth_1.adminAuth, async (req, res) => {
    try {
        const { minLot, maxLot, spreadPercent, priceLevels } = req.body;
        const result = await bot_service_1.BotService.populateAllStocks({
            minLot,
            maxLot,
            spreadPercent,
            priceLevels
        ...
        const stocksRes = await database_1.default.query('SELECT symbol FROM stocks WHERE is_active = true');
        for (const stock of stocksRes.rows) {
            await matching_engine_1.MatchingEngine.match(stock.symbol);
        }
        res.json(result);
    }
    catch (err) {
...
router.delete('/bot/clear', auth_1.adminAuth, async (req, res) => {
    try {
        const symbol = req.query.symbol;
        const result = await bot_service_1.BotService.clearBotOrders(symbol);
...
router.get('/bot/stats/:symbol', auth_1.adminAuth, async (req, res) => {
    try {
        const symbol = req.params.symbol;
        const stats = await bot_service_1.BotService.getOrderbookStats(symbol);
        res.json(stats);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.get('/bot/supply/:symbol', auth_1.adminAuth, async (req, res) => {
    try {
        const symbol = req.params.symbol;
        const supplyInfo = await bot_service_1.BotService.getStockSupplyInfo(symbol);
        res.json(supplyInfo);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.get('/health', auth_1.adminAuth, async (req, res) => {
    try {
        const healthResult = await matching_engine_1.MatchingEngine.healthCheck();
        res.json({
...
router.get('/engine/stats', auth_1.adminAuth, async (req, res) => {
    try {
        const stats = matching_engine_1.MatchingEngine.getStats();
        res.json(stats);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.post('/engine/reset-circuit', auth_1.adminAuth, async (req, res) => {
    try {
        const { symbol } = req.body;
        matching_engine_1.MatchingEngine.resetCircuitBreaker(symbol);
        res.json({
            success: true,
            message: symbol
...
router.get('/orderbook/validate', auth_1.adminAuth, async (req, res) => {
    try {
        const symbol = req.query.symbol;
        if (!symbol)
            return res.status(400).json({ error: 'Symbol diperlukan' });
        ...
        const validate = (orders) => {
            ...
            for (const o of orders) {
                try {
                    ...
                    if (!parsed.orderId || !parsed.userId || !parsed.price)
                        issues.push(o);
                }
                catch {
                    issues.push(o);
...
router.post('/engine/force-broadcast', auth_1.adminAuth, async (req, res) => {
    try {
        const { symbol } = req.body;
        if (!symbol)
            return res.status(400).json({ error: 'Symbol diperlukan' });
        await matching_engine_1.MatchingEngine.forceBroadcast(symbol);
...
// IPO MANAGEMENT
// ========================================
// POST /api/admin/ipo - Create new IPO
router.post('/ipo', auth_1.adminAuth, async (req, res) => {
    try {
        const result = await ipo_service_1.IPOService.createIPO(req.body);
        res.json(result);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
// POST /api/admin/ipo/:id/finalize - Finalize/Allocate IPO
router.post('/ipo/:id/finalize', auth_1.adminAuth, async (req, res) => {
    try {
        const id = req.params.id;
        const result = await ipo_service_1.IPOService.finalizeIPO(id);
        res.json(result);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
// DIVIDEND MANAGEMENT
// ========================================
// POST /api/admin/dividends - Distribute Dividend
router.post('/dividends', auth_1.adminAuth, async (req, res) => {
    try {
        // body: { stockId: 1, amountPerShare: 50, sessionId: ... }
        const result = await dividend_service_1.DividendService.distributeDividend(req.body);
        res.json(result);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
