...
class MatchingEngine {
    ...
    static broadcastTimers = new Map();
    static pendingBroadcasts = new Set();
    // Circuit Breaker per symbol
    static circuitBreakers = new Map();
    static CIRCUIT_FAILURE_THRESHOLD = 5;
    static CIRCUIT_RESET_TIMEOUT = 30000; // 30 seconds
    static CIRCUIT_HALF_OPEN_SUCCESS_THRESHOLD = 3;
    ...
    // Configurable limits
    static MAX_ITERATIONS_PER_CYCLE = 100;
    static LOCK_TIMEOUT_MS = 30000;
    static BROADCAST_THROTTLE_MS = 500; // 500ms throttle for better responsiveness
    ...
    static parseOrderQueue(raw, symbol, side) {
        ...
        for (let i = 0; i < raw.length; i += 2) {
            try {
                ...
                // Ensure remaining_quantity exists
                if (typeof data.remaining_quantity !== 'number') {
    ...
    static async executeTrade(buyOrder, sellOrder, matchPrice, symbol, originalBuyPrice, originalSellPrice, buyRaw, sellRaw) {
        ...
        try {
            ...
            // 1. Remove old entries from Redis FIRST (atomic)
            await Promise.all([
                redis_1.default.zrem(`orderbook:${symbol}:buy`, buyRaw),
                redis_1.default.zrem(`orderbook:${symbol}:sell`, sellRaw)
            ]);
            // 2. Add back if partially filled
            if (buyRemaining > 0) {
                const newBuyOrder = { ...buyOrder, remaining_quantity: buyRemaining };
                await redis_1.default.zadd(`orderbook:${symbol}:buy`, originalBuyPrice, JSON.stringify(newBuyOrder));
            }
            if (sellRemaining > 0) {
                const newSellOrder = { ...sellOrder, remaining_quantity: sellRemaining };
                await redis_1.default.zadd(`orderbook:${symbol}:sell`, originalSellPrice, JSON.stringify(newSellOrder));
            }
            // 3. Record trade (only for real orders)
            if (!isBuyBot && !isSellBot) {
                await client.query(`INSERT INTO trades (buy_order_id, sell_order_id, price, quantity) VALUES ($1, $2, $3, $4)`, [buyOrder.orderId, sellOrder.orderId, matchPrice, matchQty]);
            }
            ...
            // 6. Update seller balance and portfolio
            if (!isSellBot) {
            ...
            // 8. Emit notifications (non-blocking)
        ...
        try {
            ...
            const parseOrders = (raw) => {
                ...
                for (let i = 0; i < raw.length; i += 2) {
                    try {
                        const data = JSON.parse(raw[i]);
                        if (!data.remaining_quantity || data.remaining_quantity <= 0)
                            continue;
                        result.push({
                            price: parseFloat(raw[i + 1]),
            ...
            const aggregateByPrice = (orders) => {
                ...
                for (const order of orders) {
                    const currentQty = priceMap.get(order.price) || 0;
                    priceMap.set(order.price, currentQty + order.quantity);
                }
                return Array.from(priceMap.entries())
                    .map(([price, totalQty]) => ({ price, volume: totalQty }));
            };
            ...
            const validBids = bids.filter(b => b && typeof b.price === 'number' && typeof b.volume === 'number').slice(0, 20);
            const validAsks = asks.filter(a => a && typeof a.price === 'number' && typeof a.volume === 'number').slice(0, 20);
