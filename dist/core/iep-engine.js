...
class IEPEngine {
     ...
     */
    static async calculateIEP(symbol) {
        ...
        const [buyOrdersRaw, sellOrdersRaw] = await Promise.all([
            redis_1.default.zrevrange(buyKey, 0, -1, 'WITHSCORES'),
            redis_1.default.zrange(sellKey, 0, -1, 'WITHSCORES')
        ]);
        ...
        const bestCandidates = candidates.filter(c => c.volume === topCandidate.volume && c.surplus === topCandidate.surplus);
        let finalPrice = topCandidate.price;
        if (bestCandidates.length > 1) {
            // Fetch Prev Close
            const stockRes = await database_1.default.query('SELECT prev_close FROM daily_stock_data d JOIN stocks s ON s.id = d.stock_id WHERE s.symbol = $1 ORDER BY d.session_id DESC LIMIT 1', [symbol]);
     ...
     */
    static async executeIEPTrades(symbol, iep) {
        if (!iep || iep.matchVolume === 0)
            return;
        ...
        try {
            ...
            // Get eligible orders
            // Buys: score >= iepPrice
            const buyOrders = await redis_1.default.zrangebyscore(buyKey, iep.iepPrice, '+inf'); // Standard ZRANGEBYSCORE is min max
            // Redis ZRANGEBYSCORE min max.
            // Sells: score <= iepPrice
            const sellOrders = await redis_1.default.zrangebyscore(sellKey, '-inf', iep.iepPrice);
            // Sort manually to be safe (Price Priority then Time Priority)
            const parsedBuys = buyOrders.map(s => JSON.parse(s)).sort((a, b) => {
                if (b.price !== a.price)
                    return b.price - a.price; // Higher buy price first
                return a.timestamp - b.timestamp; // Earlier first
            });
            const parsedSells = sellOrders.map(s => JSON.parse(s)).sort((a, b) => {
                if (a.price !== b.price)
                    return a.price - b.price; // Lower sell price first
                return a.timestamp - b.timestamp; // Earlier first
            });
            ...
            let sellIdx = 0;
            const trades = [];
            const processedOrders = new Set(); // Order IDs modified
            ...
            const updateRedisOrder = (order, type) => {
            ...
            for (const order of parsedBuys) {
                if (processedOrders.has(order.orderId)) {
                    ...
                    const oldEntries = await redis_1.default.zrangebyscore(`orderbook:${symbol}:buy`, order.price, order.price);
            ...
            for (const order of parsedSells) {
                if (processedOrders.has(order.orderId)) {
                    const oldEntries = await redis_1.default.zrangebyscore(`orderbook:${symbol}:sell`, order.price, order.price);
                    for (const entry of oldEntries) {
                        if (entry.includes(order.orderId)) {
