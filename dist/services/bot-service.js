...
class BotService {
    ...
    static async populateOrderbook(symbol, options) {
        ...
        try {
            ...
            // Validation
            if (minLot < 1)
                throw new Error('minLot must be >= 1');
            if (maxLot < minLot)
                throw new Error('maxLot must be >= minLot');
            if (priceLevels < 1)
                throw new Error('priceLevels must be >= 1');
            // 1. Ambil info stock
            ...
            const tickSize = (0, market_logic_1.getTickSize)(referencePrice);
    ...
    static roundToTickSize(price) {
        const tick = (0, market_logic_1.getTickSize)(price);
